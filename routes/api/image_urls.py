...
from routes.dashboard import ImageURLCreateForm, ImageURLUpdateForm
from flask_wtf.csrf import validate_csrf
bp = Blueprint("image_urls", __name__)
import models

@bp.route("/image_urls/create", methods=['POST'])
@login_required
def create():
    ...
    if form.validate_on_submit():
        with models.db.app_engine().Session() as session:
            ...
            try:
                new_item = models.ImageURL(name=name, url=url, description=description)
...
@bp.route("/image_urls/update", methods=['POST'])
@login_required
def update():
    ...
    if form.validate_on_submit():
        ...
        with models.db.app_engine().Session() as session:
            try:
                url = session.query(models.ImageURL).get(form.id.data)
...
@bp.route("/image_urls/delete", methods=['POST'])
@login_required
def delete():
    form = ImageURLUpdateForm()
    session: Session
